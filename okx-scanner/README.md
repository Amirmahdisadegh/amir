...
**پیگیری بسته‌شدن پوزیشن (Mode 2):** در ابتدای هر سیکل اسکن، `executor.reconcile()`
پوزیشن‌های باز صرافی را با پوزیشن‌های ثبت‌شده مقایسه می‌کند؛ هر پوزیشنی که روی
صرافی بسته شده (اصابت SL/TP)، PnL محقق‌شده‌اش از fillهای اخیر خوانده و در شمارندهٔ
ضرر روزانه ثبت می‌شود. بدون این مرحله، Daily Loss Limit در حالت زنده هرگز حرکت
نمی‌کرد.
...
- **کارمزد و اسلیپیج** لحاظ می‌شوند: `fee_pct` روی هر طرف معامله، و `slippage_pct` روی خروج‌های استاپ (market). TP چون Limit است اسلیپیج ندارد.
- دو حالت گزارش:
  - **per-symbol**: هر کوین مستقل روی کل سرمایه (برای دیدن رفتار تک‌کوین).
  - **PORTFOLIO**: همهٔ کوین‌ها روی یک سرمایهٔ مشترک با سقف سراسری `max_open_positions` — واقعی‌ترین سنجه.
- خروجی: **Win Rate، Profit Factor، Avg R:R، Expectancy، Max Drawdown، Return%**.

کوین‌ها/تایم‌فریم/بازهٔ تست و کارمزد/اسلیپیج در بخش `backtest` تنظیم می‌شوند.
